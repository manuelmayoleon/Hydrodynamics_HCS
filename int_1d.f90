...
subroutine inicializacion1d(npart,long,temp,r,v)
    ...
    INTEGER:: i,number
    REAL(kind=8)::long
    REAL(kind=8),DIMENSION(npart):: v
    REAL(kind=8),DIMENSION(npart)::r
    REAL(kind=8)::sumvsq,temp
    REAL(kind=8)::sumv
    REAL(kind=8):: x1,x2,y1,y2
    REAL(kind=8)::size
    REAL(kind=8), parameter :: pi = 4 * atan (1.0_8)
    ...
    !!!! introducimos las variables caracteristicas del sistema      
         ...
         number=npart +1 !te da aproximadamente la mitad de las particulas para distribuirlas en x e y
         size=long/REAL(number,8) ! te divide el espacio de tal forma que las particulas se distribuyan uniformmemente
         
    !!!! establecemos las posiciones y velocidades de las particulas  
        DO  i=1,npart
            ! r(i)=i*size-long/2.0d0
            r(i)=i*size
            ! r(i,2)=0.d00
        END DO
        ...
        DO i=1,npart,2 
            ...
            !determina las velocidades a traves de una distribucion maxwelliana
            v(i)=sqrt(-2.*log(x1))*cos(2.*pi*y1)
            v(i+1)=sqrt(-2.*log(x2))*sin(2.*pi*y2)
            ! print*, v(i)
        END DO
    ...
    !!!! Escalamos las velocidades para que las temperaturas sean las deseadas
        ...
        DO  i=1,npart !!multiplicamos por la raiz cuadrada de la temperatura /temperatura promedio <v^2>/2=v^2/2n (para particulas con masa unidad)
            v(i)=sqrt(dble(npart)*temp/sumvsq)*v(i)   
            ! print*, v(i)
        END DO
        
END SUBROUTINE inicializacion1d
